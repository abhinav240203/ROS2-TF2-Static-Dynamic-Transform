...
from robot_msgs.srv import GetTransform
...
class SimpleTfKinematics(Node):
    def __init__(self):
        ...
        self.orientation_incement_ = quaternion_from_euler(0, 0, 0.05)

        self.tf_buffer_ = Buffer()
        self.tf_listener_ = TransformListener(self.tf_buffer_, self)

        self.static_transform_stamped_.header.stamp = self.get_clock().now().to_msg()
        self.static_transform_stamped_.header.frame_id = "robot_base"
        self.static_transform_stamped_.child_frame_id = "robot_top"
        ...
        self.get_logger().info("static transform between %s and %s" %(self.static_transform_stamped_.header.frame_id, self.static_transform_stamped_.child_frame_id))
    ...
    def getTransformCallback(self, req, res):
        self.get_logger().info("requesting transform %s and %s" % (req.frame_id, req.child_frame_id))
        ...
        try:
            ...
            self.get_logger().error("error between %s and %s" % (req.frame_id, req.child_frame_id))
            res.success = False
            return res
    ...
    def timerCallback(self):
        ...
        self.dynamic_transform_stamped_.child_frame_id = "robot_base"
        ...
        q = quaternion_multiply(self.last_orientation_, self.orientation_incement_)
        ...
        if self.rotations_counter_ >= 120:
            self.orientation_incement_ = quaternion_inverse(self.orientation_incement_)
            self.rotations_counter_ = 0
